...
class NN extends Component {
	...
	//}
	//componentDidUpdate() {
	//const svg = d3.select("#nn").select("#ee");
	////this.initNeuralNetwork(svg);
	...
	//}
	/* What I want to do now is pass in the logic for the  */
	render() {
				...
				<svg id="pp" width="800" height="500">
					{this.props.links.map((d, i) => (
						<path
							...
							stroke={
								this.props.weights[i] > 0 ? "lightblue" : "red"
							}
							fill="none"
							onClick={() => console.log(d)}
						></path>
					))}
					{this.props.rects.map((d) => (
						<rect
							x={d.x}
							y={d.y}
							width={32}
							height={32}
							onClick={() => console.log(d.x)}
							fill="lightgrey"
						></rect>
					))}
				</svg>
